...
That's it. Claude will research your company, pre-fill every context file with real data, ask you 6 targeted questions for what it couldn't find publicly, and write the full repo in one shot.

---

## What This Does

Instead of filling in 6 context files from blank templates, you provide a domain and Claude does the research. The output is a repo that's 70–80% complete from public information alone. You fill in the remaining 20–30% — the things only you know: your win patterns, your anti-ICP, and your deal values.
...
- Metrics like retention rate, win/loss analysis, sales cycle length, segment annual growth


Claude writes the repo first. No questions upfront. After you see the result, it offers a 3-minute refinement pass (6 questions) to replace inferred fields with your actual data. Skip it and the repo still works.
...
- Lookalikes: infer from tier 1 and tier 2, similar segments they can target but aren’t targeting. 
...
- Competitive positioning: use the research + question 6
...
### 7. `context/pricing/`
Pricing extracted from company and 2-3 top competitors’ pricing pages (use the same competitors referenced for `context/competitor-radar.md`  For each:
- Pricing model: [Per seat / Usage-based / Flat-fee or subscription / outcome or success based/ Hybrid/Freemium] Free tier or trial: [Yes/No — details]
- Indicate whether they are point players (sell only one product) or they sell bundles/packages of more than 1 product
- Unique selling points
- Key factors or trends impacting pricing (for ex., recent price wars)
-Total cost of ownership (TCO) (infer from company and competitor websites)
-Customer feedback on pricing (infer from G2 reviews)
...
Want to sharpen what was inferred? Take 3 minutes to answer 5 questions and I'll update every file with your actual data. 
...
Ask exactly these 6 questions in a single message. Do not split them across multiple prompts.

```
6 questions to sharpen your context:
...
5. How many of your products or features do they use? (e.g. 1, 2, 3, etc)

6. Competitive nuance — anything not visible publicly?
   A competitor you're seeing in most deals, or an angle that's been working?
```

After receiving the answers, update every relevant file — replace `[inferred]` fields with confirmed data, add anti-ICP and number of products or features used to the ICP definition, update signal library with the signals they named, update CLAUDE.md priorities. Then confirm what changed:

```
Updated with your answers:

- context/icp-definition.md — anti-ICP added, number of products or features used added, Tier 1 criteria sharpened
